...
const SubAgentLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/admins/sg-login`,
        { userName, password }
      );
      ...
      if (data?.user) {
        ...
        if (data.user.role === "SG") {
          navigate("/sg/sub-agent");
        } else {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-blue-500 via-blue-900 to-yellow-500">
